...
class CrosswordCreator():
    ...
    def assignment_complete(self, assignment):
        ...
        """
        for v in self.crossword.variables:
            if v not in assignment or assignment[v] not in self.crossword.words:
                return False
        return True
        # raise NotImplementedError
            ...
            for y, word_y in assignment.items():
                if x != y:
                    ...
                    if overlap is not None:
                        i, j = self.crossword.overlaps[x, y]
                        if i is not None and word_x[i] != word_y[j]:
                            return False
        return True
        #raise NotImplementedError
                ...
                if neighbor not in assignment:
                    ...
                    for word_neighbor in self.domains[neighbor]:
                        if word[i] != word_neighbor[j]:
                            count += 1
            word_and_n.append((word, count))
        word_and_n.sort(key=lambda x: x[1])
        return [word for word, _ in word_and_n]
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.crossword.variables:
            if var not in assignment.keys():
                var_and_domain.append((var, len(self.domains[var])))
        var_and_domain.sort(key=lambda x: (x[1], -len(self.crossword.neighbors(x[0]))))
        return var_and_domain[0][0]
    ...
    def backtrack(self, assignment):
        ...
        for word in self.order_domain_values(var, assignment):
            ...
            if self.consistent(new_assignment):
                ...
                if all(len(self.domains[v]) > 0 for v in self.crossword.variables):
